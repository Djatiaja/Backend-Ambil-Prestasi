...
export const getAllTeachers = async (req: Request, res: Response) => {
    try {
        ...
        const { users: teachers, meta } = await userService.getAllUsers({ role: "Teacher", isDeleted: false, limit, page });
...
export const createTeacher = async (req: Request, res: Response) => {
    try {
        ...
        if (!name || !email || !password) {
            return sendResponse({ res: res, statusCode: 400, success: false, message: "Name, email, and password are required", data: null });
        }
        // TODO: Add username generator
        // TODO: Add validation for email and password strength
